...
// @desc    Create new gallery item with cover + multiple images
// @access  Private
router.post('/', auth, upload.fields([
  ...
  try {
    ...
    if (req.files && req.files.coverImage && req.files.coverImage[0]) {
      galleryData.coverImage = '/uploads/' + req.files.coverImage[0].filename;
    }
    if (req.files && req.files.images) {
      galleryData.images = req.files.images.map(f => '/uploads/' + f.filename);
    }
...
// @desc    Update gallery item with cover + multiple images
// @access  Private
router.put('/:id', auth, upload.fields([
  ...
  try {
    ...
    if (req.files && req.files.images) {
      // Merge with existing images if keepExistingImages flag is set
      ...
      if (updateData.keepExistingImages === 'true') {
        ...
        updateData.images = [...(existing ? existing.images : []), ...newImages];
      } else {
        updateData.images = newImages;
      }
    }
    // Parse existingImages array sent from admin (for keeping some, removing others)
    if (updateData.existingImages) {
      try {
        const existing = JSON.parse(updateData.existingImages);
        const newUploads = req.files && req.files.images ? req.files.images.map(f => '/uploads/' + f.filename) : [];
        updateData.images = [...existing, ...newUploads];
      } catch (e) {
